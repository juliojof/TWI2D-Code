...
void  getAd(float *Ad, float *d, float *image_psf, int **positionPSF);

float* getInterpHessianAtPoint(int i, float *imagePSF, int **positionPSF);
                 ...
                 float dz_, float dt_, float ox_, float oz_, int niter, float epsilon_)
{
    ...
    for(k=0; k<niter; k++)
    {
        ...
        getAd(Ad, d, image_psf, positionPSF);
...
void  getAd(float *Ad, float *d, float *image_psf, int **positionPSF)
{
    ...
    for(ix=jxPSF/2; ix<nx-jxPSF/2; ix++)
        for(iz=jzPSF/2; iz<nz-jzPSF/2; iz++)
        {
            ...
            Ad[i] = 0.0f;
            A = getInterpHessianAtPoint(i, image_psf, positionPSF);
...
float* getInterpHessianAtPoint(int i, float *imagePSF, int **positionPSF)
{
    ...
    */
    ...
    p_01   = p_01/p_total;
    p_11   = p_11/p_total;
    float *A = interpK(A_00, A_01, A_10, A_11, p_00, p_01, p_10, p_11, (jzPSF+1), (jxPSF+1));
